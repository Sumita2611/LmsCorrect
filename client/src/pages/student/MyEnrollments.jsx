...
const MyEnrollments = () => {
  ...
  useEffect(() => {
    // Update local courses when enrolledCourses changes
    setLocalCourses(enrolledCourses);

    // Generate progress data for enrolled courses
  ...
  const confirmRemove = () => {
    ...
    try {
      ...
      console.log(`Removing course from enrollments: ${courseToRemove._id}`);

      // Use the removeEnrollment function from context
      removeEnrollment(courseToRemove._id);

      // No success notification, just let the UI update
    } catch (error) {
